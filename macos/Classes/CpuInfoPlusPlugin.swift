...
public class CpuInfoPlusPlugin: NSObject, FlutterPlugin {
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    freq.setStreamHandler(FreqStreamHandlerMac())
  }
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "getFrequencyTelemetryOnce":
      result(CpuInfoPlusPlugin().makeTelemetryPayloadMac())
    default:
      result(FlutterMethodNotImplemented)
    }
  }

  fileprivate func makeTelemetryPayloadMac() -> [String: Any] {
  ...
  private func socIdentity() -> [String: String] {
    ...
    m["note_cpu_implementer"] =
      "无 Android 风格 cpuinfo CPU implementer；此处为 sysctl 机型/CPU 字符串，不等于商品化 SoC 名称。"
    m["note_chip_hint"] = "芯片代际可结合 hw.machine / Apple Silicon 世代与机型公开资料。"
    if let v = sysctlString("hw.machine") { m["hw_machine"] = v }
    if let v = sysctlString("hw.model") { m["hw_model"] = v }
    if let v = sysctlString("machdep.cpu.brand_string") { m["cpu_brand_string"] = v }
    m["host_name"] = ProcessInfo.processInfo.hostName
    return m
  }
...
private class FreqStreamHandlerMac: NSObject, FlutterStreamHandler {
  ...
  func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
    ...
    let emit: () -> Void = {
      ...
      events(p.makeTelemetryPayloadMac())
    }
    emit()
